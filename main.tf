...
resource "azurerm_policy_definition" "filebased" {
  ...
  parameters          = try(jsonencode(jsondecode(file("${path.module}/${each.key}"))["properties"]["parameters"]), jsonencode({}))
  policy_rule         = try(jsonencode(jsondecode(file("${path.module}/${each.key}"))["properties"]["policyRule"]), jsonencode({}))
